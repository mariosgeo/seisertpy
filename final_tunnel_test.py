...
# 2. ενδιάμεσος χώρος-κέλυφος γύρω από το τούνελ (max area 1.5)
plc_2d.addRegionMarker([0, 3.75], marker=2, area=1.5)
...
# Δημιουργία Z-slices (προαιρετικό refinement γύρω από τα ηλεκτρόδια)
if refine_electrodes:
    refined_z_slices = []
    for z in floor_z_positions:
        refined_z_slices.extend([z - 0.2, z, z + 0.2])
else:
    refined_z_slices = list(floor_z_positions)
